...
class FishEntry {
  ...
  updateNextWindowData() {
    ...
    if (crs.length > 0) {
      ...
      this.availability.upcoming.downtime = dateFns.distanceInWordsStrict(currEnd, nextStart) + "后";
    ...
    if (crs.length > 0) {
      ...
      if (dateFns.isAfter(currStart, earthTime)) {
        ...
        this.availability.current.duration =
          dateFns.distanceInWordsStrict(earthTime, currStart)+"后";
        this.availability.current.date = currStart;
      } else {
        // The fish is currently available!
        this.isUpSoon = false; // It's already up! XD
        this.availability.current.duration =
          dateFns.distanceInWordsStrict(earthTime, currEnd) + "后结束";
        this.availability.current.date = currEnd;
      }
      this.availability.upcoming.duration = dateFns.distanceInWordsStrict(earthTime, nextStart)+"后";
...
let ViewModel = new class {
  ...
  initialize() {
    ...
    moment.updateLocale('zh-cn', {calendar: {sameElse: 'M/D dddd, Ah:mm'}});
