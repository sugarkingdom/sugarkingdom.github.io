		...
		showPage : function(options) {
			...
			if (total > 0) {
				...
				if (page > total) {
					...
					$first.on("click", {page:page}, options.onPage);
				}
				if (page > 1) {
					...
					if (hasEvent) {
						$first.on("click", {page:1}, options.onPage);
						$pre.on("click", {page:page-1}, options.onPage);
					}
				} else {
				...
				if (page - set > 2) {
					...
					if (hasEvent) {
						$li.on("click", {page:1}, options.onPage);
					}
					for (var i = page - set; i < page; i++) {
						$li = $('<li><a href="javascript:;">' + i + '</a></li>');
						$ul.append($li);
						if (hasEvent) {
							$li.on("click", {page:i}, options.onPage);
						}
					}
				} else {
				...
				if ( page + set  < total - 1) {
					...
					if (hasEvent) {
						$li.on("click", {page:total}, options.onPage);
					}
				} else {
				...
				if (page < total) {
					...
					if (hasEvent) {
						$last.on("click", {page:total}, options.onPage);
						$next.on("click", {page:page+1}, options.onPage);
					}
				} else {
				...
				if (parseInt(options.page) > total) {
					this.on("click", {page:page}, options.onPage);
					this.trigger("click");
					this.unbind("on");
				}
			} else {
