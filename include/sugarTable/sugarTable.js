	...
	$.fn.sugarTable.defaults = {
		table: { // 表参数
			...
			rowGenHandler: function name(data) {
				// console.log("sugarTable row generated!");
			},
			tableGenHandler: function name(data) {
				// console.log("sugarTable table generated!");
			},
		},
		field: { // 域参数
			text: { // 域类型-文本
			...
			select: { // 域类型-选择框
				...
				changeHandler: function name(data) {
					console.log("Welcome to Sugar Kingdom!");
				},
			},
			textarea: { // 域类型-文本框
				id: "",
		...
		init: function (options) {
			...
			if (_table.o.needSum || _table.o.needFinalSum) {
				...
				for (_table.listIndex = 0; _table.listIndex < _table.o.list.length; _table.listIndex++) {
					...
					// 构造原始行号
					_table.listData.sugar_seri = (_table.o.page - 1) * _table.o.pageSize + parseInt(_table.listIndex) + 1;
				...
				if (_table.o.needFinalSum) { // 需要额外总计行
					...
					if (_table.finalSumData !== {}) {
						...
						// 添加统计数据行号
						_table.finalSumData.sugar_seri = "总计";
